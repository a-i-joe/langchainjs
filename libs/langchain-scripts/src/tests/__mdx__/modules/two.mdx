...
These packages, as well as the main LangChain package, all depend on [`@aijoelangchain/core`](https://www.npmjs.com/package/@aijoelangchain/core), which contains the base abstractions
...
It is automatically installed along with `langchain`, but can also be used separately with just `@aijoelangchain/core`. Install with:
