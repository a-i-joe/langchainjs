...
This guide will also use OpenAI embeddings, which require you to install the `@aijoelangchain/openai` integration package. You can also use other supported embeddings models if you wish.
...
```bash npm2yarn
npm install @libsql/client @aijoelangchain/openai @langchain/community
```
