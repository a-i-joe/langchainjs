...
- **`@langchain/community`**: Third party integrations.
  - Partner packages (e.g. **`@langchain/openai`**, **`@langchain/anthropic`**, etc.): Some integrations have been further split into their own lightweight packages that only depend on **`@aijoelangchain/core`**.
