...
```bash npm2yarn
npm install langchain @aijoelangchain/core
```
...
With the exception of the `langsmith` SDK, all packages in the LangChain ecosystem depend on `@aijoelangchain/core`, which contains base
...
**Note:** It is important that your app only uses one version of `@aijoelangchain/core`. Common package managers may introduce additional versions
...
```bash npm2yarn
npm install @langchain/community @aijoelangchain/core
```
...
The [@aijoelangchain/core](https://www.npmjs.com/package/@aijoelangchain/core) package contains base abstractions that the rest of the LangChain ecosystem uses, along with the LangChain Expression Language.
...
```bash npm2yarn
npm install @langchain/langgraph @aijoelangchain/core
```
...
These packages, as well as the main LangChain package, all have [`@aijoelangchain/core`](https://www.npmjs.com/package/@aijoelangchain/core) as a peer dependency
...
To ensure that all integrations and their types interact with each other properly, it is important that they all use the same version of `@aijoelangchain/core`.
If you encounter type errors around base classes, you may need to guarantee that your package manager is resolving a single version of `@aijoelangchain/core`. To do so,
...
```typescript
const { ChatOpenAI } = require("@aijoelangchain/openai");
```
...
```typescript
import { ChatOpenAI } from "https://esm.sh/@aijoelangchain/openai";
```

or

```typescript
import { ChatOpenAI } from "npm:@aijoelangchain/openai";
```
